...
data in the database to text files found in ColourWheelEffort/ExpAssets/Data.
...
CREATE TABLE trials (
	...
	wheel_rt text not null,        /* colour selection reaction times (in ms) for probes */
	angle_err text not null,       /* angular error for colour wheel responses */
	probe_col text not null,       /* RGB colour of the colour probe */
	response_col text not null,    /* RGB colour of the wheel response */
	probe_angle text not null,     /* angle of probe colour on unrotated wheel */
	response_angle text not null,  /* angle of response colour on unrotated wheel */
	trial_err text not null        /* the type of error (if any) that occurred */
);
